...
сравниваются лицом к лицу в обоих порядках, чтобы компенсировать позиционное смещение.
...
[autoresearch](https://github.com/karpathy/autoresearch) Андрея Карпати — см. [Родословную](#родословная).
...
2. **Парный гейт keep/discard.** Каждый кандидат сравнивается лицом к лицу с текущим
   чемпионом. Чтобы компенсировать позиционное смещение (LLM склонны предпочитать
   первый вариант), судья оценивает оба порядка: `[Кандидат, Чемпион]` и
   `[Чемпион, Кандидат]`. Мутация **остаётся, только если кандидат набирает больше
   голосов, чем чемпион**; при равенстве она откатывается. Уверенные-но-худшие
   переписывания не уезжают в прод.
