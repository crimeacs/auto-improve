...
fn conf() -> Conf {
    Conf {
        window_title: "auto-improve · climb".to_string(),
        window_width: 900,
        window_height: 560,
        high_dpi: true,
        ..Default::default()
...
async fn main() {
    ...
    if dump {
        ...
        for r in &climb.rows {
            println!(
                "  it{:>2} {:>3} {}",
                r.iter as i64, r.score as i64, r.status
            );
        }
        return;
    }
...
fn draw_climb(c: &Climb, t: f32) {
    ...
    draw_text(
        format!("{} · score vs iteration · live", c.tag),
        36.0,
        70.0,
        16.0,
        DIM,
    ...
    if c.rows.is_empty() {
        draw_text(
            "waiting for the loop to write results…",
            36.0,
            112.0,
            18.0,
            DIM,
    ...
    draw_text(
        "green keep · red reset · gold retry        Esc quit",
        x0,
        sh - 14.0,
        13.0,
        DIM,
